...
//	}
	local.svc = new query()
	local.svc.setSQL('SELECT LogDBErrID = NEXT VALUE FOR LogDBID') // I'm having a hard time executing an update followed by a select in Railo.
	local.obj = local.svc.execute()
	local.LogDBErrID = local.obj.getResult().LogDBErrID
